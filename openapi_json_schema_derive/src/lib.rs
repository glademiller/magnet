...
pub fn derive_json_schema(input: TokenStream) -> TokenStream {
    impl_json_schema(input).unwrap_or_else(|error| panic!("{}", error))
}
...
fn impl_json_schema(input: TokenStream) -> Result<TokenStream> {
    ...
    let impl_ast = match parsed_ast.data {
        Data::Struct(s) => impl_json_schema_struct(parsed_ast.attrs, s)?,
        Data::Enum(e) => impl_json_schema_enum(parsed_ast.attrs, e)?,
        Data::Union(u) => impl_json_schema_union(parsed_ast.attrs, u)?,
    };
